...
export const getMovie = ({ params, response }) => {
  ...
  const movie = movies.filter((movie) =>
    movie.title.toLowerCase() === params.title.toLowerCase()
  );
  if (movie.length) {
    response.status = 200;
...
export const updateMovie = async ({ params, request, response }) => {
  ...
  const temp = movies.filter((movie) =>
    movie.rank.toString() === params.rank.toString()
  );
  const body = await request.body();
  console.log("req body : ", body);
  ...
  if (temp.length) {
    ...
    temp[0].domestic_gross = domestic_gross
      ? domestic_gross
      : temp[0].domestic_gross;
    response.status = 200;
    response.body = {
  ...
  response.body = response.body = {
    message: `Cannot find rank ${params.rank}`,
  };
};
